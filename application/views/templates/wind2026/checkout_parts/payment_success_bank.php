...
<div class="mx-auto max-w-3xl px-4 py-14">
    <div class="rounded-3xl bg-white p-8 shadow-sm ring-1 ring-slate-200">
        <?php
        $selectedPaymentType = isset($_SESSION['payment_type']) ? $_SESSION['payment_type'] : 'Bank';
        $transferContent = isset($_SESSION['order_id']) ? 'ORDER-' . $_SESSION['order_id'] : '';
        if ($selectedPaymentType === 'Bank' && isset($_SESSION['order_id'])) {
            $transferContent = 'BANK-ORDER-' . $_SESSION['order_id'];
        }
        ?>
        <div class="flex items-start gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">
                    <?php if ($selectedPaymentType === 'VisaCard') { ?>
                        Visa Card Payment
                    <?php } else { ?>
                        <?= lang('bank_payment') ?>
                    <?php } ?>
                ...
                <div class="grid grid-cols-1 gap-3 text-sm text-slate-800">
                    <?php if ($selectedPaymentType === 'VisaCard') { ?>
                        <div>
                            ...
                            <div class="mt-1 font-semibold">Visa Card</div>
                        </div>
                        <div>
                            <div class="text-xs font-semibold uppercase tracking-wider text-slate-500">Instruction</div>
                            <div class="mt-1">Our team will contact you to confirm secure Visa card payment for order <b><?= $_SESSION['order_id'] ?></b>.</div>
                        </div>
                    <?php } else { ?>
                        <div>
                        ...
                        <div>
                            ...
                            <div class="mt-1 font-semibold"><?= $transferContent ?></div>
                        </div>
                    <?php } ?>
                </div>
            </div>
